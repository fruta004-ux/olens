...
export function AddClientDialog({ open: controlledOpen, onOpenChange, stage, trigger, onSuccess }: AddClientDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error("기존 거래처 추가 실패:", error)
      alert("기존 거래처 추가에 실패했습니다.")
    }
  }
    ...
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      <SheetContent side="right" className="w-full sm:max-w-2xl overflow-y-auto p-0">
        <div className="sticky top-0 z-10 bg-background border-b px-6 py-6">
          <SheetHeader>
            <SheetTitle className="text-2xl font-bold">기존 거래처 추가</SheetTitle>
            <SheetDescription className="text-sm">기존 거래처 정보를 입력하여 관리하세요.</SheetDescription>
          </SheetHeader>
        </div>
